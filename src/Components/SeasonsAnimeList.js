...
const SeasonsAnimeList = ({ apiUrl }) => {
  const [seasonsAnimeList, setSeasonsAnimeList] = useState([]);

  useEffect(() => {
    const fetchAnimeData = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_BASEURL}/seasons/now?limit=12`
        );
        setSeasonsAnimeList(response.data.data);
      } catch (error) {
        console.log(error);
      }
    };
  ...
  return (
    <div className="bg-color-dark">
      <div className="flex justify-between items-center md:p-4 md:px-8 p-3 text-start">
        <h1 className="text-2xl font-bold text-color-primary">Top Seasons</h1>
        <Link
          to={``}
